...
describe("profile editor model", () => {
  test("loads hub and tip profiles from manifest", () => {
    ...
    assert.equal(profiles.hub_profile.control_points.length, 4);
    assert.equal(profiles.tip_or_shroud_profile.control_points.length, 4);
  });
  ...
  test("updates one control point deterministically", () => {
    ...
    assert.deepEqual(profiles.hub_profile.control_points[1], [260, 60]);
